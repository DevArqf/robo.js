...
describe('CodeAgent Integration', () => {
	...
	describe('explain mode', () => {
		it('should produce grounded answers without edits', async () => {
	...
	describe('plan mode', () => {
		it('should produce acceptance criteria and may interrupt', async () => {
			...
			mockLLM.addResponse({
				content: JSON.stringify({
					...
					mustPass: ['build'],
					plan: [
						{ step: 1, title: 'Create command', description: 'Create the command file', files: [] }
					]
				})
			})
	...
	describe('execute mode', () => {
		it('should implement and verify changes', async () => {
			...
			mockLLM.addResponse({
				content: JSON.stringify({
					...
					mustPass: [],
					plan: [
						{ step: 1, title: 'Create file', description: 'Create hello.ts', files: ['/src/hello.ts'] }
					]
				})
			})
			...
			mockLLM.addResponse(
				MockResponses.fsWrite('/src/hello.ts', 'export const hello = () => "Hello, World!"')
			)
	...
	describe('budget exhaustion', () => {
		it('should stop and report status when budget exceeded', async () => {
	...
	describe('Question Gate', () => {
		it('should interrupt and resume correctly', async () => {
		...
		it('should not repeat side effects on resume', async () => {
	...
	describe('run management', () => {
		it('should track runs correctly', async () => {
		...
		it('should support multiple concurrent runs', async () => {
			mockLLM.addResponse({ content: '{}' })
		...
		it('should abort runs correctly', async () => {
			mockLLM.addResponse({ content: '{}' })
