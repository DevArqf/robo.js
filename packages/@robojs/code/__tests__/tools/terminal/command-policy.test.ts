...
function createPolicy(overrides: Partial<AgentPolicy> = {}): AgentPolicy {
	return {
		...
		commandArgPolicy: {
			disallow: [
				{ command: 'node', argsPrefix: ['-e', '--eval', '-p', '--print'] }
			],
			requireApproval: [
...
describe('terminal_run argument policies', () => {
	describe('disallow patterns', () => {
		...
		it('should allow safe node usage', async () => {
			...
			const result = await terminalRunTool.execute(
				{ command: 'node', args: ['index.js', '--flag', 'value'] },
				context
			)
...
describe('terminal_session_start command policy', () => {
	...
	it('should allow approved session commands', async () => {
		...
		const result = await terminalSessionStartTool.execute(
			{ command: 'node', args: ['--inspect', 'app.js'] },
			context
		)
