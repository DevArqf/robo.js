...
export default async (request: RoboRequest) => {
	// 1. Validate method
	if (request.method !== 'PATCH' && request.method !== 'DELETE' && request.method !== 'GET') {
	...
	// 7. Validate interaction has a response message
	...
	// 8. Get the message
	const message = session.state.getMessage(interaction.responseMessageId)
	if (!message) {
		return new Response(
			JSON.stringify({
				error: 'Unknown Message',
	...
	if (request.method === 'GET') {
		...
		return handlePatch(request, session, interaction, message, appId, token)
	} else {
		return handleDelete(session, interaction, message, appId, token)
	}
}
...
function handleDelete(
	...
	// Clear responseMessageId on interaction
	...
	session.recorder.record(
		...
		{
			endpoint: `DELETE /webhooks/${appId}/${token}/messages/@original`,
			method: 'DELETE',
			interactionId: interaction.id
		}
	)
