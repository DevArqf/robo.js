...
export interface KeyConfig {
	 ...
	 * - 'parentOrFilename': Use parent folder if nested, otherwise filename
	 *   (events/ready.ts → "ready", events/messageCreate/chat.ts → "messageCreate")
	 */
	 ...
	 * Receives the generated key and the original path segments for full flexibility.
	 * @param key - The key generated by the style setting
	 * @param segments - The original path segments (e.g., ['messageCreate', 'chat'] for events/messageCreate/chat.ts)
	 * @example
	 * // Use parent folder for nested files
	 * transform: (key, segments) => segments.length > 1 ? segments[0] : key
	 */
