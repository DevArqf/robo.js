...
**Important:** The `as const` assertion is required for TypeScript to infer literal types from your config.
...
```typescript
// In plugin: src/robo/cli/commands/my-plugin/status.ts
...
- **[@robojs/cli Plugin](../plugins/cli)** - Build standalone CLI applications
- **[Robo CLI Reference](./robo)** - Built-in command reference
