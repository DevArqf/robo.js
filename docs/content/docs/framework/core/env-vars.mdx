...
```bash meta='title=".env.production"'
DEBUG_STUFF="false"
EXAMPLE_VARIABLE="Hello, production!"
```

```bash meta='title=".env.development"'
DEBUG_STUFF="true"
...
```javascript meta='title="/src/commands/secret.js"'
export default () => {
	return process.env.EXAMPLE_VARIABLE ?? 'No secret found!'
}
```
...
<Cards>
  <Card href="./mode" title="🎭 Mode" description="Define profiles for your Robo session." />
</Cards>
...
```javascript meta='title="/src/events/_start.js"'
export default () => {
...
Plus, you get type checking with **[TypeScript](./typescript)**!

<Cards>
  <Card href="./typescript" title="📘 TypeScript" description="Write better code with type safety." />
</Cards>
