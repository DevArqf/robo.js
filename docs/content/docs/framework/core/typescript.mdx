...
```typescript title="/src/commands/ts-command.ts"
...
```typescript title="/src/commands/easy-peasy.ts"
// Before
...
Although Robo.js and monorepos naturally get along, there's a secret sauce for importing bits across projects – [the magic of TypeScript's path aliases](#smoothing-imports-with-path-aliases-%F0%9F%9A%80) ✨.
...
<Callout type="info" title="Just one golden rule!">
