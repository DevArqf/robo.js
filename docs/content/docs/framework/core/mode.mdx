...
**Modes** are a way to define "profiles" for your **Robo** session. Each with its own config(s), environment variables, and code.
...
```bash title="Mode: development"
robo dev
```

```bash title="Mode: production"
robo start
```

```bash title="Mode: beta"
