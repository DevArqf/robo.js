...
**[Client](./client.mdx)** network requests made by your **Discord Activity** are "sandboxed" through **[Discord's Proxy](https://discord.com/developers/docs/activities/development-guides#activity-proxy-considerations)**.
...
You can automate your **CSP** compliance by using **[@robojs/patch](../plugins/patch.mdx)** to patch URLs in your code with zero effort. All **Robo.js** activity projects come with this plugin pre-installed by default.
...
<Callout type="info" title="Don't actually use GitHub as a CDN">
...
As a last resort, you can proxy requests through your **[Web Server](./server.mdx)**. This is useful when you need to make requests to a specific port or handle requests that the **Discord Proxy** can't, but it's not always the best solution.
...
Don't trust info coming from the **Embedded App SDK** as truth. There could be an impostor among us. Call the Discord API directly from your app's **[Web Server](./server.mdx)** with the **OAuth2** token you received during **[Authentication](./authentication.mdx)** if you need information that's not sus.

<Callout type="warn" title="Sanitize Your Inputs">
