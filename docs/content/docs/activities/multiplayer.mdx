...
```package-install
@colyseus/core @colyseus/schema @colyseus/ws-transport colyseus.js
```

From here on, you'll need to:

1. **Create a Server**: We recommend extending `NodeEngine` in **[@robojs/server](../plugins/server.mdx)** to work with Colyseus.
